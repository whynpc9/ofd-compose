...
Ten spacing cases, including semantic and semantics-free inputs, verify unrelated columns, a leading boundary space, a trailing boundary space, and widely separated printable/combining glyphs within one cluster. Stock high-level text is literal-exact; pre-fix PDFs in `fixtures/spacing-reference` are independent visual references, and CI requires unchanged pixels.

Within each semantic or fallback context, glyphs are separated unless an actual trailing/leading space mapping connects their boundary. This retains real spaces across font runs while isolating non-space gaps even inside mixed-space objects. State wrappers and paint sequence are preserved; geometry does not decide logical grouping.
...
A single glyph mapping to two or more ASCII spaces returns object-level `UNSUPPORTED_FEATURE`: the pinned reader collapses that mapping. This differs from the accepted Worker input, which has two separate space glyphs. Two mixed-space regressions preserve `o fff` without an inserted separator, with/without semantics; their pre-fix paint is pixel-identical.
