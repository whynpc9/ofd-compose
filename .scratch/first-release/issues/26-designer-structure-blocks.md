...
- [ ] ConditionalBlock/RepeatBlock/RepeatRowGroup 包裹与解除、数据源/作用域/重复键/空值策略修改进入同一历史域；与文本及绑定编辑混合撤销/重做，原子恢复 AST、身份映射、投影与逻辑选区；显示文字不变也建立检查点
- [ ] 结构变更后重复展开实例身份与模板 nodeId 分离；undo/redo、新编辑分支和延迟预览共同验证，不能用全量重载清历史作为首版完成替代
