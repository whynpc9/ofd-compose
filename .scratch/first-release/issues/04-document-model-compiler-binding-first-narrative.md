...
- 日期值只识别 ISO 8601 形态（`date.ts`）：形态匹配但字段非法（`2024-13-45`、`2024-01-01Z`）与非 ISO 文本一样退回原文，不让 Temporal 的 RangeError 逃出 `bind()`；排序/极值比较 ISO 日期时按绝对时刻比较（带偏移的值换算到同一时刻，不带偏移的按 UTC 字面），不是字符串比较。
- PR #2 评审（Codex）处理：(a) `sort`/`maxby`/`minby` 的键缺失不再直接当最小值进入比较——strict-1 下整体 Missing 并报 `BINDING_MISSING`（dataPath 指向首个缺失键的原始位置，如 `items[1].score`），legacy-compat-1 下按 null 比较并记 `LEGACY_SEMANTIC_CHANGE`（rule `missing-as-null`，details 含 `op`/`missingCount`）；显式 `null` 键仍是合法最小值。(b) `settings.timeZone` 在 `bind()` 入口用 temporal-polyfill 校验（`isValidTimeZone`；接受 IANA 名与 `+08:00` 固定偏移），非法值产生模板级 `MODEL_INVALID` error（phase `bind`），带偏移的日期按 UTC 回退取字段使文档仍可产出。(c) JSON 数字超出 IEEE-754 精度（如 `9007199254740993`）在进入 `bind(data: JsonValue)` 前已经丢失，非 binding-core 可修复；`toDecimal` 已接受十进制字符串作为精确表示，数据契约层面（字符串金额 vs 无损 JSON 解析）留待数据 API 票（issue 21）决定，未在本票改动。
- 代码评审（两轴：标准 / spec）已跑并处理：合并重复的路径/步骤错误包装（`segmentsOf`/`sourceOf`/`withStepContext`）、`ResolvedInputControlSchema` 复用模板侧 `InputControlSchema`、`bind()` 的共享状态收成 `BindContext`。未处理但记录的判断项：`patternCache` 以字符串键缓存编译期已校验的模式（可在 issue 05 把解析后的模式直接放入 AST）；`null|format:...` 等派生值的 `valueState` 记为 `value` 而非保留 `null` 来源。
- 测试规模：document-model 9、template-compiler 14、binding-core 61、golden-corpus 20（node）/ 6（browser）、gates 20；`pnpm lint`、`pnpm typecheck`、`pnpm build`、`pnpm test`、`pnpm test:browser` 全绿。`pnpm check:licenses` 的 pnpm 侧手工核对：新增生产依赖 `@sinclair/typebox`、`decimal.js`、`temporal-polyfill`（MIT）及其传递依赖 `temporal-spec`（Apache-2.0）、`temporal-utils`（MIT）均在白名单；NuGet 侧因本机无 `global.json` 要求的 .NET SDK 版本带未能运行，由 CI 覆盖（.NET 侧无改动）。
- 与 ADR-0001 / spec 的偏差与判断性补充（供人工确认）：
  1. `temporal-polyfill` 采用 1.0.4（ADR 原写 0.3.x）；已在 ADR-0001 追加「修订记录」把版本带改为 1.0.x（1.x 为 Temporal 提案 Stage 3 稳定 API，许可不变）。若人工否决该修订需同时改回 `packages/binding-core/package.json`。
  2. 新增诊断码 `MODEL_INVALID`（spec §12 未列，用于 TypeBox 结构校验失败），并沿用 `UNSUPPORTED_FEATURE` 表示未知必需扩展。
  3. 日期 `format:date` 的字段化格式化从 issue 05 提前实现（corpus 叙述类用例不依赖它，但编译器需要在编译期校验日期模式）；tzdata 版本来源记录、预算（`REPEAT_LIMIT`/`RESOURCE_LIMIT`）仍留给 issue 05。
  4. strict-1 下 `count` 作用于非数组没有专用诊断码（spec §12 无类型不匹配码），暂借用 `EXPRESSION_UNSUPPORTED`（phase `bind`，details.rule `count-non-array`）；如需独立码位请在 spec §12 追加。
  5. 兼容政策目前只在文档级 `settings.bindingPolicyVersion` 切换；spec §6 所说的"节点级附带兼容政策"留给 issue 05 与迁移票（结构节点落地后才有多层作用域）。同理 `scope-fallback` 目前只能通过 `evaluateExpression` 的显式 `Scope` 触发，`bind()` 对段落级 DynamicText 始终是 `current === root`。
  6. 结构块（循环/条件/表格/图片）不在本票范围，`narrative-template.ts` 遇到控制块即判定非叙述类。排序采用稳定排序（输入序号 tie-break，spec §6），不复刻旧引擎 `Sort()+Reverse()` 的不稳定性。
