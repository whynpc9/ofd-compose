...
internal static class IrValidation
{
    ...
    internal static JsonDocument Parse(ReadOnlyMemory<byte> bytes, string digest, WriterLimits limits,CancellationToken cancellationToken=default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        Require(bytes.Length <= limits.JsonBytes, "RESOURCE_LIMIT", "ir", "IR byte budget exceeded");
        bytes = bytes.ToArray();cancellationToken.ThrowIfCancellationRequested();
        var scan = new Utf8JsonReader(bytes.Span, new JsonReaderOptions { MaxDepth = 64 });
        int tokens = 0, strings = 0;
        while (scan.Read())
        {
            if((tokens&1023)==0)cancellationToken.ThrowIfCancellationRequested();
        ...
        cancellationToken.ThrowIfCancellationRequested();string actualDigest=Digest(bytes.Span);cancellationToken.ThrowIfCancellationRequested();
        Require(actualDigest == digest, "IR_DIGEST_MISMATCH", "ir", "IR byte digest mismatch");
        ...
        try
        {
            ...
            var node=JsonNode.Parse(bytes.Span);cancellationToken.ThrowIfCancellationRequested();
            bool valid=Schema.Value.Evaluate(node,new EvaluationOptions {OutputFormat=OutputFormat.Flag}).IsValid;
            cancellationToken.ThrowIfCancellationRequested();Require(valid,"IR_SCHEMA","ir","Canonical Layout IR schema validation failed");
            Require(Encoding.UTF8.GetString(bytes.Span) == Canonical(ir,false,cancellationToken), "IR_NON_CANONICAL", "ir", "Wire bytes must be canonical JSON");
            References(ir, limits,cancellationToken);cancellationToken.ThrowIfCancellationRequested();
            return document;
        }
        catch { document.Dispose(); throw; }
    }
    // Match the shared canonical serializer. Numbers are checked, never used to re-hash the transport.
    internal static string Canonical(JsonElement value,bool omitId=false,CancellationToken cancellationToken=default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        return value.ValueKind switch
        {
            JsonValueKind.Object => "{"+string.Join(",",CheckEach(value.EnumerateObject(),cancellationToken).Where(p=>!omitId||p.Name!="id")
                .OrderBy(p=>p.Name,StringComparer.Ordinal).Select(p=>Quote(p.Name,cancellationToken)+":"+Canonical(p.Value,false,cancellationToken)))+"}",
            JsonValueKind.Array => "["+string.Join(",",CheckEach(value.EnumerateArray(),cancellationToken).Select(v=>Canonical(v,false,cancellationToken)))+"]",
            JsonValueKind.String => Quote(value.GetString()!,cancellationToken),
    ...
    private static string Quote(string value,CancellationToken cancellationToken)
    {
        var b=new StringBuilder("\"");int index=0;
        ...
        cancellationToken.ThrowIfCancellationRequested();return b.Append('"').ToString();
    }
    private static IEnumerable<T> CheckEach<T>(IEnumerable<T> source,CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        foreach(var item in source){cancellationToken.ThrowIfCancellationRequested();yield return item;}
        cancellationToken.ThrowIfCancellationRequested();
    }
    ...
    private static void Unique(IEnumerable<string> ids, string path,CancellationToken cancellationToken)
    {
        var seen = new HashSet<string>(StringComparer.Ordinal);
        foreach (string id in CheckEach(ids,cancellationToken)) Require(seen.Add(id), "IR_DUPLICATE_ID", path, "Duplicate identity");
    }
    private static void OrderedDefinitions(JsonElement list, string prefix, bool allowEqualContent = false,CancellationToken cancellationToken=default)
    {
        string? previous = null;
        int index = 0;
        foreach (var item in CheckEach(list.EnumerateArray(),cancellationToken))
        {
            string content = Canonical(item, true,cancellationToken);
    ...
    private static void References(JsonElement ir, WriterLimits limits,CancellationToken cancellationToken)
    {
        ...
        foreach (var page in CheckEach(ir.A("pages"),cancellationToken)) foreach (var obj in CheckEach(page.A("objects"),cancellationToken))
        {
            objectsCount++;
        ...
        foreach (var state in CheckEach(ir.A("graphicsStates"),cancellationToken)) if (state.Has("clip")) commands += state.P("clip").P("commands").GetArrayLength();
        ...
        var clipCounts = CheckEach(ir.A("graphicsStates"),cancellationToken).ToDictionary(s => s.S("id"), s => s.Has("clip") ? s.P("clip").P("commands").GetArrayLength() : 0);
        ...
        OrderedDefinitions(ir.P("resources"), "r",cancellationToken:cancellationToken); OrderedDefinitions(ir.P("graphicsStates"), "s",cancellationToken:cancellationToken);
        OrderedDefinitions(ir.P("markers"), "m", allowEqualContent: true,cancellationToken:cancellationToken);
        foreach(var state in CheckEach(ir.A("graphicsStates"),cancellationToken)) if(state.Has("clip")) PathCommands(state.P("clip"),state.S("id")+"/clip",cancellationToken);
        var resources = CheckEach(ir.A("resources"),cancellationToken).ToDictionary(r => r.S("id"));
        var states = CheckEach(ir.A("graphicsStates"),cancellationToken).Select(s => s.S("id")).ToHashSet();
        var fontGlyphSets=resources.Values.Where(r=>r.S("kind")=="font"&&r.Has("glyphIdMap"))
            .ToDictionary(r=>r.S("id"),r=>CheckEach(r.A("glyphIdMap"),cancellationToken).Select(g=>g.N("original")).ToHashSet());
        var objects = new Dictionary<string, (JsonElement Object, JsonElement Page)>();
        var pages = new Dictionary<string, JsonElement>();
        int pageIndex = 0;
        foreach (var page in CheckEach(ir.A("pages"),cancellationToken))
        {
            ...
            foreach (var obj in CheckEach(page.A("objects"),cancellationToken))
            {
                ...
                if(obj.S("kind")=="path")PathCommands(obj,obj.S("id"),cancellationToken);
                if(obj.Has("clip"))PathCommands(obj.P("clip"),obj.S("id")+"/clip",cancellationToken);
                if (obj.S("kind") == "text")
                {
                    Require(resources.TryGetValue(obj.S("fontId"), out var font) && font.S("kind") == "font", "IR_REFERENCE", obj.S("id"), "Missing font");
                    Text(obj,cancellationToken);
                    if (fontGlyphSets.TryGetValue(font.S("id"),out var map))
                    {
                        foreach (var glyph in CheckEach(obj.A("glyphs"),cancellationToken)) Require(map.Contains(glyph.N("glyphId")), "IR_RESOURCE", obj.S("id"), "Unmapped glyph");
                    }
                }
        ...
        foreach (var r in resources.Values.Where(r => r.S("kind") == "font" && r.Has("glyphIdMap")))
        {
            ...
            foreach (var m in CheckEach(r.A("glyphIdMap"),cancellationToken))
            {
                Require(m.N("original") > previous && subsets.Add(m.N("subset")), "IR_RESOURCE", r.S("id"), "Invalid subset map"); previous = m.N("original");
            }
        }
        Unique(CheckEach(ir.A("semantics"),cancellationToken).Select(s => s.S("objectId")), "semantics",cancellationToken);
        double reading = -1;
        foreach (var sem in CheckEach(ir.A("semantics"),cancellationToken))
        {
            ...
            foreach (string field in new[] { "pageIndex", "sectionId", "sectionSourceId" }) if (sem.Has(field))
            {
                ...
                Require(Canonical(sem.P(field),false,cancellationToken) == Canonical(target.Page.P(pageField),false,cancellationToken), "IR_REFERENCE", "semantics", "Page/section mismatch");
            }
            if (sem.Has("sourceText")) Source(sem.P("sourceText"));
            if (sem.Has("sourceRanges")) foreach (var source in CheckEach(sem.A("sourceRanges"),cancellationToken))
            {
        ...
        foreach (var marker in CheckEach(ir.A("markers"),cancellationToken)) Require(pages.ContainsKey(marker.S("pageId")) && (!marker.Has("objectId") || objects.TryGetValue(marker.S("objectId"), out var target) && target.Page.S("id") == marker.S("pageId")), "IR_REFERENCE", marker.S("id"), "Invalid marker target");
    ...
    private static void PathCommands(JsonElement path,string location,CancellationToken cancellationToken)
    {
        bool open=false;
        foreach(var command in CheckEach(path.A("commands"),cancellationToken))
        {
    ...
    private static void Text(JsonElement obj,CancellationToken cancellationToken)
    {
        ...
        foreach (var c in CheckEach(obj.A("clusters"),cancellationToken))
        {
            ...
            foreach (var index in CheckEach(c.A("glyphIndices"),cancellationToken))
            {
