...
{
    internal static void Validate(byte[] bytes, Dictionary<string, (int Offset, int Length)> tables, int glyphs)
    {
        ...
        for(int g=0;g<glyphs;g++)
        {
            ...
            if(contours>=0)
            {
                ...
                for(int c=1;c<contours;c++)Require(U16(record,at+c*2)>U16(record,at+(c-1)*2),"IR_RESOURCE","font","Invalid contour endpoints");
    ...
    private static void Cff(ReadOnlySpan<byte> cff,int glyphs)
    {
        Require(cff[0]==1 && cff[2]>=4 && cff[2]<=cff.Length && cff[3] is >=1 and <=4,"IR_RESOURCE","font","Invalid CFF header");
        int at=cff[2]; var names=Index(cff,ref at);var top=Index(cff,ref at);Index(cff,ref at);Index(cff,ref at);
        Require(names.Count==1 && top.Count==1,"IR_RESOURCE","font","Expected one static CFF face");
        var dict=Dict(cff.Slice(top[0].Start,top[0].Length));
        ...
        var strings=Index(cff,ref charStrings);Require(strings.Count==glyphs,"IR_RESOURCE","font","CFF glyph count mismatch");
        ...
        {
            int fdAt=Offset(1236);var fonts=Index(cff,ref fdAt);
            Require(fonts.Count is >0 and <=256,"IR_RESOURCE","font","CFF FDArray count");
            foreach(var font in fonts)Private(cff,Dict(cff.Slice(font.Start,font.Length)));
            ...
            if(format==0)
            {
                ...
                for(int g=0;g<glyphs;g++)Require(cff[select+g]<fonts.Count,"IR_RESOURCE","font","CFF FD reference");
            }
            else
            {
    ...
    private static void Private(ReadOnlySpan<byte> cff,Dictionary<int,int[]> dict)
    {
        ...
        var privateDict=Dict(cff.Slice(values[1],values[0]));
    ...
    private static Dictionary<int,int[]> Dict(ReadOnlySpan<byte> dict)
    {
        ...
        for(int i=0;i<dict.Length;)
        {
            ...
            else if(b==30){bool end=false;while(i<dict.Length&&!end){int pair=dict[i++];end=(pair&15)==15||(pair>>4)==15;}Require(end,"IR_RESOURCE","font","CFF real number");operands.Add(0);}
            else
            {
