...
internal sealed class PdfObjects(int limit, CancellationToken cancellationToken = default)
{
    ...
    internal int Stream(byte[] bytes, string entries = "", bool compress = true)
    {
        ...
        if(compress)
        {
            ...
            using(var z = new ZLibStream(buffer, CompressionLevel.SmallestSize, true))
                for(int at=0;at<bytes.Length;at+=65536){CheckCancellation();z.Write(bytes.AsSpan(at,Math.Min(65536,bytes.Length-at)));}
    ...
    internal byte[] Finish(int root, string digest)
    {
        ...
        for(int i=0;i<objects.Count;i++) { CheckCancellation(); offsets[i]=stream.Position; Write($"{i+1} 0 obj\n"); stream.Write(objects[i] ?? throw new InvalidDataException()); Write("\nendobj\n"); }
        long xref=stream.Position; Write($"xref\n0 {objects.Count+1}\n0000000000 65535 f \n");
        foreach(long offset in offsets) { CheckCancellation(); Write(offset.ToString("D10",System.Globalization.CultureInfo.InvariantCulture)+" 00000 n \n"); }
        Write($"trailer\n<< /Size {objects.Count+1} /Root {root} 0 R /ID [<{digest[..32]}> <{digest[..32]}>] >>\nstartxref\n{xref}\n%%EOF\n");
        CheckCancellation();var output=stream.ToArray();CheckCancellation();return output;
    }
}
