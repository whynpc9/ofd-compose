...
public static partial class SourceContainer
{
    ...
    private static Dictionary<string,RenderedObject> SemanticMap(JsonElement root, IReadOnlyDictionary<string, string[]> objectMap, Dictionary<string, byte[]> entries, Dictionary<string,(string Kind,string Digest)> links, IReadOnlyDictionary<string,string> resourceMap, ContainerBudget budget, BarcodeGeometryResolver? barcodeGeometryResolver, NumberingLabelsResolver? numberingLabelsResolver,SourceRenderResolver? sourceRenderResolver,out SourceReplayIdentity replayIdentity,SourceReplayIdentity? expectedReplayIdentity=null)
    {
        ...
        replayIdentity=SourceRenderValidation.ValidatePages(root,entries,links.ToDictionary(item=>item.Key,item=>item.Value.Digest),sourceRenderResolver,renderProof,budget,expectedReplayIdentity);
        return objects;
    }
}
