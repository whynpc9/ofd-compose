...
public sealed class FallbackSpacingTests
{
    ...
    [Theory][InlineData("columns","o f","o f")][InlineData("semantic-runs","o","f")][InlineData("leading","o"," f")][InlineData("trailing","o ","f")]
    public async Task Unsemantic_objects_preserve_only_their_explicit_whitespace(string name,string left,string right)
    {
        ...
        n["pages"]![0]!["objects"]=objects;n["semantics"]=name=="semantic-runs"?new JsonArray(new JsonObject{["objectId"]="p0o0",["nodeId"]="same",["readingOrder"]=0},new JsonObject{["objectId"]="p0o1",["nodeId"]="same",["readingOrder"]=1}):new JsonArray();n["markers"]=new JsonArray();using var semanticDoc=JsonDocument.Parse(n["semantics"]!.ToJsonString());n["identity"]!["semanticDigest"]=WriterTests.Digest(Encoding.UTF8.GetBytes(IrValidation.Canonical(semanticDoc.RootElement,false,TestContext.Current.CancellationToken)));using var parsed=JsonDocument.Parse(n.ToJsonString());byte[] ir=Encoding.UTF8.GetBytes(IrValidation.Canonical(parsed.RootElement,false,TestContext.Current.CancellationToken));
