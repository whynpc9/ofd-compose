  ...
  private apply(state: State, step: OperationNode): State {
    ...
    switch (step.op) {
      case "sort": {
        ...
        const keys = value.map((item) => this.resolveSegments(item, step.key, undefined).value);
      ...
      case "minby": {
        ...
        let best = 0;
        let bestKey = this.resolveSegments(value[0] as JsonValue, step.key, undefined).value;
        for (let i = 1; i < value.length; i++) {
          const key = this.resolveSegments(value[i] as JsonValue, step.key, undefined).value;
          const cmp = compareValues(key, bestKey);
          if ((step.op === "maxby" && cmp > 0) || (step.op === "minby" && cmp < 0)) {
            best = i;
            bestKey = key;
          }
        }
